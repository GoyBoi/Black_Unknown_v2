// components/PerformanceAlertDashboard.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { 
  calculatePerformanceAlertDashboard, 
  ...
  PerformanceAlertDashboardData
} from '@/lib/performance-alert-dashboard';

const PerformanceAlertDashboard = () => {
  const [analytics, setAnalytics] = useState<PerformanceAlertDashboardData | null>(null);
  ...
  // Load performance alert dashboard data
  useEffect(() => {
    const loadData = async () => {
      ...
      // Calculate analytics
      const analytics = calculatePerformanceAlertDashboard();
  ...
  if (!analytics) {
    return (
      <div className="text-center py-12 text-foreground/60">
        <p>Unable to load performance alert dashboard data</p>
      </div>
    );
  }
    ...
    <div className="bg-background text-foreground rounded-lg border border-foreground/10">
      <div className="p-6 border-b border-foreground/10">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold">Performance Alert Dashboard</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 p-6 border-b border-foreground/10">
        {Object.entries(analytics.coreWebVitalsStatus).map(([vital, status]) => (
          <div 
            key={vital} 
            className={`p-4 rounded-lg border ${
              status === 'good' ? 'border-green-500/30 bg-green-500/10' :
              status === 'needs-improvement' ? 'border-yellow-500/30 bg-yellow-500/10' :
              'border-red-500/30 bg-red-500/10'
            }`}
          >
            <div className="text-center">
              <div className="text-lg font-bold capitalize">{vital}</div>
              <div className={`text-sm font-medium ${
                status === 'good' ? 'text-green-500' :
                status === 'needs-improvement' ? 'text-yellow-500' : 'text-red-500'
              }`}>
                {status.replace('-', ' ')}
              </div>
            </div>
                ...
                {[
                  ...
                  <div key={index} className="bg-foreground/5 rounded-lg border border-foreground/10 p-4">
                    ...
                    <div className="flex justify-between text-xs text-foreground/60">
                      <span>0</span>
                      <span>{item.metric === 'CLS' ? item.threshold.toFixed(3) : formatDuration(item.threshold)}</span>
                    </div>
                  </div>
                ))}
              </div>
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="w-32 bg-foreground/20 rounded-full h-2.5">
                          <div 
                            ...
                            style={{ width: `${(metric.count / analytics.alertsByMetric[0].count) * 100}%` }}
                          ></div>
                        </div>
                      </div>
                    </div>
                  ...
                  {analytics.performanceTrends.map((day, index) => (
                    <div key={index} className="flex flex-col items-center flex-1">
                      <div className="flex items-end justify-center h-64 space-y-1">
                        ...
                        ></div>
                        <div 
                          className="w-1/5 bg-purple-500/50 rounded-t hover:opacity-90 transition-opacity"
...
export default PerformanceAlertDashboard;