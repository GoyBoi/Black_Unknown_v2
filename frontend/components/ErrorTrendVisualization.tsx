// components/ErrorTrendVisualization.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { 
  calculateErrorTrendVisualization, 
  getErrorEvents, 
  ...
  ErrorTrendVisualizationData
} from '@/lib/error-trend-visualization';

const ErrorTrendVisualization = () => {
  const [analytics, setAnalytics] = useState<ErrorTrendVisualizationData | null>(null);
  ...
  // Load error trend visualization data
  useEffect(() => {
    const loadData = async () => {
      ...
      // Calculate analytics
      const analytics = calculateErrorTrendVisualization();
  ...
  if (!analytics) {
    return (
      <div className="text-center py-12 text-foreground/60">
        <p>Unable to load error trend visualization data</p>
      </div>
    );
  }
    ...
    <div className="bg-background text-foreground rounded-lg border border-foreground/10">
      <div className="p-6 border-b border-foreground/10">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold">Error Trend Visualization</h2>
            <p className="text-foreground/80 mt-1">Visualize and analyze error trends over time</p>
          </div>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b border-foreground/20">
                      ...
                      <th className="text-left py-2 px-4">Occurrences</th>
                      ...
                      <tr key={index} className="border-b border-foreground/10 hover:bg-foreground/5">
                        ...
                        <td className="py-3 px-4">
                          <span className={`flex items-center ${
                            error.change > 0 ? 'text-red-500' : 'text-green-500'
                          }`}>
                            {error.change > 0 ? '↑' : '↓'} {Math.abs(error.change).toFixed(2)}%
                          </span>
                        </td>
                      </tr>
                    ))}
...
export default ErrorTrendVisualization;