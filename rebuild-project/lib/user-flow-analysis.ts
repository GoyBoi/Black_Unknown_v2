// lib/user-flow-analysis.ts

// Define types for user flow analysis
...
export interface UserFlowPath {
  ...
  duration: number;
  conversion?: {
    type: 'purchase' | 'signup' | 'download' | 'other';
    value?: number;
  };
  deviceInfo?: {
    userAgent: string;
...
export const addUserFlowEvent = (event: Omit<UserFlowEvent, 'id' | 'timestamp'>): UserFlowEvent => {
  ...
  // Find the user's current session path
  const path = userFlowPaths.find(p => p.sessionId === event.sessionId && !p.endTime);
...
export const calculateUserFlowAnalytics = (timeRange?: { start: number; end: number }): UserFlowAnalysis => {
  ...
  const avgDuration = totalDuration / totalPaths;
  
  // Calculate conversion rate
  const convertedPaths = paths.filter(p => p.conversion).length;
  const conversionRate = (convertedPaths / totalPaths) * 100;
  
  // Calculate bounce rate (paths with only 1 page view)
  const bouncedPaths = paths.filter(p => p.path.length <= 1).length;
  const bounceRate = (bouncedPaths / totalPaths) * 100;
  
  // Calculate average pages per path
  const totalPages = paths.reduce((sum, path) => sum + path.path.length, 0);
  const avgPagesPerPath = totalPages / totalPaths;
  ...
  const topExitPages = Array.from(exitMap.entries())
    .map(([page, exits]) => ({
      page,
      exits,
      exitRate: (exits / totalPaths) * 100
    }))
    .sort((a, b) => b.exits - a.exits)
    .slice(0, 10);
  
  // Calculate user flow trends
  const dateMap = new Map<string, { paths: number; durations: number; conversions: number }>();
  paths.forEach(path => {
    const date = new Date(path.startTime).toISOString().split('T')[0]; // YYYY-MM-DD
    const current = dateMap.get(date) || { paths: 0, durations: 0, conversions: 0 };
  ...
  const userFlowTrends = Array.from(dateMap.entries())
    .map(([date, data]) => ({
      date,
      totalPaths: data.paths,
      avgDuration: data.paths > 0 ? data.durations / data.paths : 0,
      conversionRate: (data.conversions / data.paths) * 100
    }))
  ...
  const geoMap = new Map<string, { paths: number; conversions: number }>();
  paths.forEach(path => {
    if (path.location?.country) {
      const current = geoMap.get(path.location.country) || { paths: 0, conversions: 0 };
      current.paths++;
      if (path.conversion) {
        current.conversions++;
      }
  ...
  const funnelData = [
    { step: 'Home', visitors: Math.floor(totalPaths * 0.9), conversionRate: 100 },
    { step: 'Category', visitors: Math.floor(totalPaths * 0.6), conversionRate: 66.7 },
    { step: 'Product', visitors: Math.floor(totalPaths * 0.4), conversionRate: 66.7 },
    { step: 'Cart', visitors: Math.floor(totalPaths * 0.25), conversionRate: 62.5 },
    { step: 'Checkout', visitors: Math.floor(totalPaths * 0.15), conversionRate: 60 },
    { step: 'Confirmation', visitors: Math.floor(totalPaths * 0.12), conversionRate: 80 }
  ];
...
// Function to get user flow path by ID
export const getUserFlowPathById = (id: string): UserFlowPath | undefined => {
  return userFlowPaths.find(path => path.id === id);
};
...
export const generateMockUserFlowData = () => {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    // Generate a path
    const path: string[] = ['/']; // Always start at home
