// lib/user-flow-visualization.ts

// Define types for user flow visualization
...
export interface UserFlowVisualizationData {
...
// Function to calculate user flow visualization data
export const calculateUserFlowVisualization = (timeRange?: { start: number; end: number }): UserFlowVisualizationData => {
  ...
  // Calculate flow visualization data
  const allPages = new Set<string>();
  paths.forEach(path => {
    path.path.forEach(page => allPages.add(page));
  });
  
  const nodes = Array.from(allPages).map((page, index) => ({
    id: `node_${index}`,
    label: page,
    count: paths.filter(p => p.path.includes(page)).length
  }));
  
  const edges = userFlowMap.map(transition => ({
    source: nodes.find(n => n.label === transition.from)?.id || '',
    target: nodes.find(n => n.label === transition.to)?.id || '',
    count: transition.count
  })).filter(edge => edge.source && edge.target); // Only include edges with valid source and target
...
export const getUserFlowPaths = (limit?: number, filter?: { 
  userId?: string; 
  sessionId?: string; 
  ...
  for (let i = 0; i < 150; i++) {
    ...
    path.forEach((pageUrl, index) => {
      events.push({
        ...
        metadata: {
          viewport: {
            width: [375, 768, 1024, 1440][Math.floor(Math.random() * 4)],
            height: [667, 1024, 768, 900][Math.floor(Math.random() * 4)]
          }
        }
      });
    });
